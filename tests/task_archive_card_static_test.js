...
has(html, /function archivedRowsFor\(type\)[\s\S]*?if\(Array\.isArray\(archive\[key\]\)\) return archive\[key\];[\s\S]*?filter\(r => archivedValue_Build13_4\(r\['בארכיון'\]\)/, 'archive renderer uses archive-specific rows before falling back to DATA tasks');
