...
has(gs, /function BH13_4_isArchivedValue_[\s\S]*?value === true[\s\S]*?toLowerCase\(\)[\s\S]*?normalized === 'כן'[\s\S]*?normalized === 'true'[\s\S]*?normalized === '1'/, 'server archive module normalizes historical archived values without mutating the sheet');
...
has(html, /function archivedRowsFor\(type\)[\s\S]*?if\(Array\.isArray\(archive\[key\]\)\)\{[\s\S]*?return archive\[key\];[\s\S]*?filter\(r => archivedValue_Build13_4\(r\['בארכיון'\]\)/, 'archive renderer uses archive-specific rows before falling back to DATA tasks');
