...
const taskHeaders = ['מזהה משימה', 'מזהה פרויקט', 'תאריך', 'יוצר אירוע', 'אחראי', 'בארכיון'];
const taskRows = [
  ['T1', 'P1', '2026-08-16', '', 'אברהם ליברמן מנהל', ''],
  ['T2', 'P2', '2026-08-16', '', '', ''],
  ['T3', 'P3', '2026-08-16', 'דוד', 'משה', ''],
  ['T4', 'P4', '2026-08-16', '', 'משה', ''],
  ['T5', 'P5', '2026-08-16', '', '', '']
];
...
[
  'BH_calendarOwnerForTask_', 'BH_calendarColorIndex_', 'BH_FAST_currentUser_', 'BH_FAST_trackRangeRead_',
...
const expected = ['אברהם', 'משה', 'דוד', 'משה', 'ללא יוצר'];
taskRows.forEach((row, index) => {
  ...
  assert.strictEqual(sandbox.BH_calendarOwnerForTask_(task, project), expected[index], `canonical precedence case ${index + 1}`);
});
...
for (const task of payload.tasks) {
  assert.strictEqual(task['יוצר אירוע'], task['יומן']);
...
expected.forEach(owner => assert.ok(payload.calendarCreatorsAllowed.includes(owner)));
