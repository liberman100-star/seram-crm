...
api.apply({entityId:'t1',record:task,calendarRecord:{'מזהה משימה':'t1',calendarMembers:['A','B']},dashboardPatch:{entity:'task',countDelta:1,taskBucket:'none'},calendarCreatorsAdded:['A']},'task');
assert.equal(context.DATA.tasks[0],task);assert.deepEqual(context.DATA.calendarTasks[0].calendarMembers,['A','B']);
api.apply({entityId:'c1',record:{'מזהה איש קשר':'c1'},dashboardPatch:{entity:'contact',countDelta:1}},'contact');
api.apply({entityId:'p1',record:{'מזהה פרויקט':'p1'},affectedLinks:[{'מזהה שיוך':'l1'}],dashboardPatch:{entity:'project',countDelta:1}},'project');
